var url = function () {
    function r(r, c) {
        ...
        if (r = r.toString(), t = c.match(/^mailto:([^\/].+)/)) o.protocol = "mailto", o.email = t[1]; else {
            if ((t = c.match(/(.*?)\/#\!(.*)/)) && (c = t[1] + t[2]), (t = c.match(/(.*?)#(.*)/)) && (o.hash = t[2], c = t[1]), o.hash && r.match(/^#/)) return e(r, o.hash);
            if ((t = c.match(/(.*?)\?(.*)/)) && (o.query = t[2], c = t[1]), o.query && r.match(/^\?/)) return e(r, o.query);
            if ((t = c.match(/(.*?)\:?\/\/(.*)/)) && (o.protocol = t[1].toLowerCase(), c = t[2]), (t = c.match(/(.*?)(\/.*)/)) && (o.path = t[2], c = t[1]), o.path = (o.path || "").replace(/^([^\/])/, "/$1"), r.match(/^[\-0-9]+$/) && (r = r.replace(/^([^\/])/, "/$1")), r.match(/^\//)) return a(r, o.path.substring(1));
            if ((t = (t = a("/-1", o.path.substring(1))) && t.match(/(.*?)\.([^.]+)$/)) && (o.file = t[0], o.filename = t[1], o.fileext = t[2]), (t = c.match(/(.*)\:([0-9]+)$/)) && (o.port = t[2], c = t[1]), (t = c.match(/(.*?)@(.*)/)) && (o.auth = t[1], c = t[2]), o.auth && (t = o.auth.match(/(.*)\:(.*)/), o.user = t ? t[1] : o.auth, o.pass = t ? t[2] : void 0), o.hostname = c.toLowerCase(), "." === r.charAt(0)) return a(r, o.hostname);
            n() && (t = o.hostname.match(n())) && (o.tld = t[3], o.domain = t[2] ? t[2] + "." + t[3] : void 0, o.sub = t[1] || void 0), o.port = o.port || ("https" === o.protocol ? "443" : "80"), o.protocol = o.protocol || ("443" === o.port ? "https" : "http")
        }
    ...
    function a(r, c) {
        ...
        return t === r ? o : o[(r = parseInt(r.substring(1), 10)) < 0 ? o.length + r : r - 1]
    }

    function e(r, c) {
        for (var t, o = r.charAt(0), n = c.split("&"), a = [], e = {}, m = [], i = r.substring(1), s = 0, h = n.length; s < h; s++) if ("" !== (a = (a = n[s].match(/(.*?)=(.*)/)) || [n[s], n[s], ""])[1].replace(/\s/g, "")) {
            if (a[2] = (t = a[2] || "", decodeURIComponent(t.replace(/\+/g, " "))), i === a[1]) return a[2];
            (m = a[1].match(/(.*)\[([0-9]+)\]/)) ? (e[m[1]] = e[m[1]] || [], e[m[1]][m[2]] = a[2]) : e[a[1]] = a[2]
        }
        return o === r ? e : e[i]
    }
...
const store = {
    ...
    reformatList(allowList) {
        ...
        for (const siteOwner in allowList) {
            ...
            if (String(firstParties) == 'undefined') {
    ...
    async _writeSecond(CookieArr, CurrentUrl) {
        ...
        if (ExistenceDB_Arr.indexOf(CheckUrl) != -1) {
            ...
            ExtraTP = NowTP.filter(function (v) {
                return PreviousTP.indexOf(v) == -1
            });
            FewerTP = PreviousTP.filter(function (v) {
                return NowTP.indexOf(v) == -1
            });
